...
// Fetch asset details and its NF - Case Number field value
async function getAssetWithCaseNumber(assetId, token) {
  ...
  // Find the NF - Case Number field (match by name, case-insensitive)
...
export default async (req) => {
  ...
  try {
    ...
    // Support two modes:
    // 1. Direct: { asset_id, case_number } — caller provides both
    // 2. Auto-detect: { asset_id } — we fetch the case number from the DAM field

    let assetId = body.asset_id || body.id;
    let caseNumber = body.case_number;
    let assetName = body.asset_name || body.name;
    ...
    // If no case_number provided, try to fetch from DAM
    ...
    if (!existing.assets.find((a) => a.id === assetId)) {
      existing.assets.push({
        ...
        dam_url:
          assetUrl || `https://app.cmp.optimizely.com/assets/${assetId}`,
